-- ============================================================
-- ContAuditAI — analizar_tenant v13
-- Lee configuracion_tenant al inicio y usa los umbrales
-- personalizados en lugar de valores hardcodeados:
--   · discrepancia_minima   → umbral near-exact (paso 7a)
--   · dias_factura_vencida  → corte HUERFANO_XML vs FACTURA_VENCIDA
--   · monto_materialidad    → mínimo para exigir evidencia (paso 10)
-- Si el tenant no tiene config, usa los defaults del sistema.
-- ============================================================
...
BEGIN
  ...
  SELECT
    COALESCE(discrepancia_minima,  5.00),
    COALESCE(dias_factura_vencida, 30),
    COALESCE(monto_materialidad,   20000.00)
  INTO v_disc_min, v_dias_venc, v_monto_mat
  FROM contauditai.configuracion_tenant
  WHERE tenant_id = v_tenant_id;

  -- Si no existe fila de config, las variables ya tienen el default declarado arriba
