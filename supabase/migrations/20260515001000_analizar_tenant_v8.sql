-- ============================================================
-- ContAuditAI — analizar_tenant v8
-- Fix: PPD_SIN_CRP solo para facturas emitidas por el tenant
-- ============================================================
...
BEGIN
  ...
  -- ── 4. PPD sin CRP — solo facturas emitidas por el tenant ────
  INSERT INTO contauditai.alertas_riesgo
    (tenant_id, tipo_alerta, severidad, descripcion, uuid_referencia)
  SELECT
  ...
  WHERE c.tenant_id = v_tenant_id
    ...
    AND c.rfc_emisor = v_tenant_rfc          -- solo PPD que yo emití
    AND NOT EXISTS (
  ...
  -- No filtra por rfc_emisor: la presencia del UUID en el banco
  -- es evidencia suficiente sin importar el RFC configurado.
  FOR v_cfdi IN
