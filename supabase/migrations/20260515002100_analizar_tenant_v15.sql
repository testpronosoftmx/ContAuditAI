-- ============================================================
-- ContAuditAI — analizar_tenant v15
-- Fix: paso 10 MATERIALIDAD_FALTANTE usaba nombre de tabla
-- incorrecto ('evidencia_materialidad') y columna incorrecta
-- ('uuid_cfdi'). Tabla real: materialidad_evidencias, columna: cfdi_uuid.
-- Bug causaba que el motor siempre generara alertas aunque
-- hubiera evidencias documentadas en el vault.
-- ============================================================
...
DECLARE
  ...
  v_bajas        INT;
  v_score        NUMERIC;
  -- Umbrales de configuración (con defaults)
  v_disc_min     NUMERIC  := 5.00;
  v_dias_venc    INTEGER  := 30;
  v_monto_mat    NUMERIC  := 20000.00;
BEGIN
  ...
  -- ── 0b. Leer configuración personalizada ─────────────────────
  -- SELECT INTO sin fila devuelve NULL — restauramos defaults explícitamente
  SELECT discrepancia_minima, dias_factura_vencida, monto_materialidad
  INTO v_disc_min, v_dias_venc, v_monto_mat
  FROM contauditai.configuracion_tenant
  WHERE tenant_id = v_tenant_id;

  v_disc_min  := COALESCE(v_disc_min,  5.00);
  v_dias_venc := COALESCE(v_dias_venc, 30);
  v_monto_mat := COALESCE(v_monto_mat, 20000.00);
  ...
  -- ── 7a. Near-exact: diferencia ≤ v_disc_min (retención posible)
  FOR v_cfdi IN
    SELECT uuid, total, isr_retenido, concepto, fecha_emision
    FROM contauditai.cfdi_comprobantes
    WHERE tenant_id = v_tenant_id AND tipo_comprobante = 'I'
  ...
  LOOP
    ...
    FROM contauditai.transacciones_bancarias
    WHERE tenant_id = v_tenant_id AND tipo = 'Ingreso' AND conciliado = FALSE
      AND monto BETWEEN v_net - v_disc_min AND v_net - 0.01
      AND fecha_operacion BETWEEN v_cfdi.fecha_emision - INTERVAL '7 days'
                              AND v_cfdi.fecha_emision + INTERVAL '45 days'
    ORDER BY ABS(EXTRACT(EPOCH FROM (fecha_operacion - v_cfdi.fecha_emision)))
    LIMIT 1;
  ...
  LOOP
    ...
    FROM contauditai.transacciones_bancarias
    WHERE tenant_id = v_tenant_id AND tipo = 'Ingreso' AND conciliado = FALSE
      AND monto BETWEEN v_net * 0.50 AND v_net - v_disc_min - 0.01
      AND fecha_operacion BETWEEN v_cfdi.fecha_emision - INTERVAL '7 days'
                              AND v_cfdi.fecha_emision + INTERVAL '45 days'
    ORDER BY ABS(EXTRACT(EPOCH FROM (fecha_operacion - v_cfdi.fecha_emision)))
    LIMIT 1;
  ...
  -- ── 8. VENTANA_72H (fija por CFF, no configurable) ───────────
  INSERT INTO contauditai.alertas_riesgo
    (tenant_id, tipo_alerta, severidad, descripcion, uuid_referencia)
  SELECT
  ...
  -- ── 10. MATERIALIDAD_FALTANTE (usa v_monto_mat configurable) ─
  INSERT INTO contauditai.alertas_riesgo
    (tenant_id, tipo_alerta, severidad, descripcion, uuid_referencia)
  SELECT
  ...
  WHERE c.tenant_id = v_tenant_id
    AND c.tipo_comprobante = 'I'
    AND c.total >= v_monto_mat
    AND NOT EXISTS (
      SELECT 1 FROM contauditai.materialidad_evidencias e
      WHERE e.cfdi_uuid = c.uuid::TEXT AND e.tenant_id = v_tenant_id
    )
    AND NOT EXISTS (
      SELECT 1 FROM contauditai.alertas_riesgo ar
  ...
  -- ── 11. HUERFANO_XML (usa v_dias_venc configurable) ──────────
  INSERT INTO contauditai.alertas_riesgo
    (tenant_id, tipo_alerta, severidad, descripcion, uuid_referencia)
  SELECT
  ...
  WHERE c.tenant_id = v_tenant_id
    ...
    AND c.fecha_emision >= NOW() - (v_dias_venc || ' days')::INTERVAL
    AND NOT EXISTS (
      SELECT 1 FROM contauditai.alertas_riesgo ar
  ...
  -- ── 12. FACTURA_VENCIDA (usa v_dias_venc configurable) ───────
  INSERT INTO contauditai.alertas_riesgo
    (tenant_id, tipo_alerta, severidad, descripcion, uuid_referencia)
  SELECT
  ...
  RETURN jsonb_build_object(
    ...
    'config',   jsonb_build_object(
      'discrepancia_minima',  v_disc_min,
      'dias_factura_vencida', v_dias_venc,
      'monto_materialidad',   v_monto_mat
    )
  );
END;
$$;
...
GRANT SELECT ON contauditai.configuracion_tenant TO authenticated;
