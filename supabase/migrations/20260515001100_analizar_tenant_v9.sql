-- ============================================================
-- ContAuditAI — analizar_tenant v9
-- Fix: PPD_SIN_CRP también detecta facturas PPD recibidas por
--      el tenant sin CRP (proveedores que no emitieron complemento)
-- ============================================================
...
BEGIN
  ...
  -- Cubre dos casos:
  --   a) Yo emití PPD a un cliente y no recibí el complemento de pago
  --   b) Un proveedor me facturó PPD y no emitió el complemento (no puedo deducir)
  INSERT INTO contauditai.alertas_riesgo
    (tenant_id, tipo_alerta, severidad, descripcion, uuid_referencia)
  SELECT
  ...
  -- ── 6. Match exacto: monto + fecha (solo emitidos por tenant) ─
  FOR v_cfdi IN
    SELECT uuid, total, fecha_emision
    FROM contauditai.cfdi_comprobantes
    WHERE tenant_id = v_tenant_id
  ...
  -- ── 7. Match parcial: 10%-95% (solo emitidos por tenant) ─────
  FOR v_cfdi IN
    SELECT uuid, total, fecha_emision
    FROM contauditai.cfdi_comprobantes
    WHERE tenant_id = v_tenant_id
  ...
  -- ── 11. HUERFANO_XML: CFDI reciente sin cobro (< 30 días) ────
  INSERT INTO contauditai.alertas_riesgo
    (tenant_id, tipo_alerta, severidad, descripcion, uuid_referencia)
  SELECT
