-- ============================================================
-- ContAuditAI — v10
-- Agrega concepto, isr_retenido, iva_retenido a CFDIs
-- Motor usa (total - isr_retenido) como monto neto esperado
-- Descriptions incluyen concepto del CFDI
-- ============================================================

-- ── 1. Nuevas columnas ────────────────────────────────────────
ALTER TABLE contauditai.cfdi_comprobantes
  ADD COLUMN IF NOT EXISTS concepto     TEXT,
  ADD COLUMN IF NOT EXISTS isr_retenido NUMERIC(15,2) NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS iva_retenido NUMERIC(15,2) NOT NULL DEFAULT 0;

-- ── 2. insertar_cfdis actualizado ────────────────────────────
...
BEGIN
  ...
  INSERT INTO contauditai.cfdi_comprobantes
    (uuid, tenant_id, rfc_emisor, rfc_receptor, fecha_emision,
     ...
     concepto, isr_retenido, iva_retenido)
  SELECT
    (c->>'uuid')::UUID,
    v_tenant_id,
    c->>'rfc_emisor',
...
BEGIN
  ...
  -- ── 5. Match Perfecto: UUID en banco ─────────────────────────
  FOR v_cfdi IN
  ...
  LOOP
    ...
    WHERE tenant_id = v_tenant_id
      AND tipo = 'Ingreso'
      AND conciliado = FALSE
      AND monto BETWEEN v_net * 0.10 AND v_net * 0.95
      AND fecha_operacion BETWEEN v_cfdi.fecha_emision - INTERVAL '7 days'
                              AND v_cfdi.fecha_emision + INTERVAL '45 days'
    ORDER BY ABS(EXTRACT(EPOCH FROM (fecha_operacion - v_cfdi.fecha_emision)))
    LIMIT 1;
