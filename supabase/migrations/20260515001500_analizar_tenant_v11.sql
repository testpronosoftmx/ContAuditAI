-- ============================================================
-- ContAuditAI — analizar_tenant v11
-- Fix: umbral de match parcial (BAJA) de 10%-95% → 50%-99%
--   Un depósito de $1,000 contra una factura de $10,000 (10%)
--   es casi siempre un falso positivo. El nuevo umbral inferior
--   de 50% garantiza que el match representa un pago parcial
--   genuino. El límite superior sube a v_net-0.01 para capturar
--   pagos de $9,999 contra factura de $10,000 sin duplicar el
--   match exacto del paso 6.
-- ============================================================
...
BEGIN
  ...
  -- ── 4. PPD sin CRP — emitidas O recibidas por el tenant ──────
  INSERT INTO contauditai.alertas_riesgo
    (tenant_id, tipo_alerta, severidad, descripcion, uuid_referencia)
  SELECT
  ...
  -- ── 6. Match exacto: monto neto + fecha (solo emitidos) ──────
  FOR v_cfdi IN
    SELECT uuid, total, isr_retenido, concepto, fecha_emision
    FROM contauditai.cfdi_comprobantes
    WHERE tenant_id = v_tenant_id
      AND tipo_comprobante = 'I'
      AND rfc_emisor = v_tenant_rfc
  ...
  -- ── 7. Match parcial: 50%-99% del monto neto (solo emitidos) ─
  -- Umbral inferior 50%: por debajo es casi siempre falso positivo.
  -- Umbral superior v_net-0.01: el match exacto ya fue capturado en paso 6.
  FOR v_cfdi IN
    SELECT uuid, total, isr_retenido, concepto, fecha_emision
    FROM contauditai.cfdi_comprobantes
    WHERE tenant_id = v_tenant_id
  ...
  LOOP
    ...
    WHERE tenant_id = v_tenant_id
      AND tipo = 'Ingreso'
      AND conciliado = FALSE
      AND monto BETWEEN v_net * 0.50 AND v_net - 0.01
      AND fecha_operacion BETWEEN v_cfdi.fecha_emision - INTERVAL '7 days'
                              AND v_cfdi.fecha_emision + INTERVAL '45 days'
    ORDER BY ABS(EXTRACT(EPOCH FROM (fecha_operacion - v_cfdi.fecha_emision)))
    LIMIT 1;
    ...
    IF v_tx_id IS NOT NULL THEN
      ...
      IF NOT EXISTS (
        ...
        VALUES (
          ...
          ' de $' || TO_CHAR(v_net, 'FM999,999,990.00') ||
          ' (' || ROUND((v_tx_monto / v_net) * 100, 1)::text || '% cobrado)' ||
          ' — Verificar CRP o cobro pendiente',
          v_cfdi.uuid
        );
      END IF;
    END IF;
  ...
  -- ── 11. HUERFANO_XML: factura reciente sin cobro (≤ 30 días) ─
  INSERT INTO contauditai.alertas_riesgo
    (tenant_id, tipo_alerta, severidad, descripcion, uuid_referencia)
  SELECT
