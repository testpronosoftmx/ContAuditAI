-- ============================================================
-- ContAuditAI — analizar_tenant v6
-- Fix 1: descripciones de alertas enriquecidas (RFC, monto, días)
-- Fix 2: motor respeta alertas ya Resueltas/Ignoradas al re-ejecutar
-- ============================================================
...
BEGIN
  ...
  WHERE c.tenant_id = v_tenant_id
    AND NOT EXISTS (
      ...
      WHERE ar.tenant_id = v_tenant_id
        AND ar.tipo_alerta = 'EFOS_DETECTADO'
        AND ar.uuid_referencia = c.uuid
        AND ar.estado IN ('Resuelto', 'Ignorado')
    );
  ...
  WHERE c.tenant_id = v_tenant_id
    ...
    AND NOT EXISTS (
      SELECT 1 FROM contauditai.alertas_riesgo ar
      WHERE ar.tenant_id = v_tenant_id
        AND ar.tipo_alerta = 'PPD_SIN_CRP'
        AND ar.uuid_referencia = c.uuid
        AND ar.estado IN ('Resuelto', 'Ignorado')
    );

  -- ── 4. Match exacto: monto + fecha ───────────────────────────
  FOR v_cfdi IN
  ...
  LOOP
    ...
    IF v_tx_id IS NOT NULL THEN
      ...
      -- Alerta solo si no fue ya Resuelta/Ignorada
      IF NOT EXISTS (
        SELECT 1 FROM contauditai.alertas_riesgo ar
        WHERE ar.tenant_id = v_tenant_id
          AND ar.tipo_alerta = 'DISCREPANCIA_BANCARIA'
          AND ar.uuid_referencia = v_cfdi.uuid
          AND ar.estado IN ('Resuelto', 'Ignorado')
      ) THEN
        INSERT INTO contauditai.alertas_riesgo
          (tenant_id, tipo_alerta, severidad, descripcion, uuid_referencia)
        VALUES (
          ...
          ' — Recibido $' || TO_CHAR(v_tx_monto, 'FM999,999,990.00') ||
          ' de $' || TO_CHAR(v_cfdi.total, 'FM999,999,990.00') ||
          ' (' || ROUND((v_tx_monto / v_cfdi.total) * 100, 1)::text || '% cobrado)',
          v_cfdi.uuid
        );
      END IF;
    END IF;
  ...
  WHERE con.tenant_id = v_tenant_id
    ...
    AND NOT EXISTS (
      SELECT 1 FROM contauditai.alertas_riesgo ar
      WHERE ar.tenant_id = v_tenant_id
        AND ar.tipo_alerta = 'VENTANA_72H'
        AND ar.uuid_referencia = c.uuid
        AND ar.estado IN ('Resuelto', 'Ignorado')
    );
  ...
  WHERE con.tenant_id = v_tenant_id
    ...
    AND NOT EXISTS (
      SELECT 1 FROM contauditai.alertas_riesgo ar
      WHERE ar.tenant_id = v_tenant_id
        AND ar.tipo_alerta = 'CONCILIACION_CRUCE_MES'
        AND ar.uuid_referencia = c.uuid
        AND ar.estado IN ('Resuelto', 'Ignorado')
    );

  -- ── 8. MATERIALIDAD_FALTANTE ─────────────────────────────────
  INSERT INTO contauditai.alertas_riesgo
    (tenant_id, tipo_alerta, severidad, descripcion, uuid_referencia)
  SELECT
  ...
  WHERE c.tenant_id = v_tenant_id
    ...
    AND NOT EXISTS (
      SELECT 1 FROM contauditai.alertas_riesgo ar
      WHERE ar.tenant_id = v_tenant_id
        AND ar.tipo_alerta = 'MATERIALIDAD_FALTANTE'
        AND ar.uuid_referencia = c.uuid
        AND ar.estado IN ('Resuelto', 'Ignorado')
    );
  ...
  WHERE c.tenant_id = v_tenant_id
    ...
    AND NOT EXISTS (
      SELECT 1 FROM contauditai.alertas_riesgo ar
      WHERE ar.tenant_id = v_tenant_id
        AND ar.tipo_alerta = 'HUERFANO_XML'
        AND ar.uuid_referencia = c.uuid
        AND ar.estado IN ('Resuelto', 'Ignorado')
    );
  ...
  WHERE c.tenant_id = v_tenant_id
    ...
    AND NOT EXISTS (
      SELECT 1 FROM contauditai.alertas_riesgo ar
      WHERE ar.tenant_id = v_tenant_id
        AND ar.tipo_alerta = 'FACTURA_VENCIDA'
        AND ar.uuid_referencia = c.uuid
        AND ar.estado IN ('Resuelto', 'Ignorado')
    );

  -- ── 11. INGRESO_NO_FACTURADO ──────────────────────────────────
  -- Sin uuid_referencia: no aplica guardia Resuelto/Ignorado por CFDI
  INSERT INTO contauditai.alertas_riesgo
  ...
  SELECT
    ...
    CASE WHEN tb.rfc_contraparte IS NOT NULL
         THEN ' — RFC: ' || tb.rfc_contraparte ELSE '' END ||
    CASE WHEN tb.clave_rastreo IS NOT NULL
         THEN ' — Clave: ' || tb.clave_rastreo ELSE '' END
